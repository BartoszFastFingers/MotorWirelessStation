...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMaster_ServiceHdle;                    /**< Master_SERVICE handle */
  uint16_t  CustomMaster_RxHdle;                  /**< MASTER_RX handle */
  uint16_t  CustomMaster_Tx11Hdle;                  /**< MASTER_TX1 handle */
...
/* Private variables ---------------------------------------------------------*/
uint16_t SizeMaster_Rx = 20;
uint16_t SizeMaster_Tx11 = 20;
...
#define COPY_MASTER_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_MASTER_RX_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_MASTER_TX1_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x01,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomMaster_RxHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            ...
            extern uint8_t counter;
                ...
                uint8_t received_char = attribute_modified->Attr_Data[0];

                counter++;

                NotifyCharData[0] = counter;

                Custom_STM_App_Update_Char(CUSTOM_STM_MASTER_TX11, NotifyCharData);
        ...
        case ACI_GATT_READ_PERMIT_REQ_VSEVT_CODE :
          ...
          if (read_req->Attribute_Handle == (CustomContext.CustomMaster_Tx11Hdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for Master_SERVICE +
   *                                2 for MASTER_RX +
   *                                2 for MASTER_TX1 +
   *                              = 5
   *
  ...
  COPY_MASTER_SERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMaster_ServiceHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: Master_SERVICE, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: Master_SERVICE , handle = 0x%04x \n\r", CustomContext.CustomMaster_ServiceHdle);
  }

  /**
   *  MASTER_RX
   */
  COPY_MASTER_RX_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMaster_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMaster_Rx,
                          CHAR_PROP_WRITE,
                          ...
                          &(CustomContext.CustomMaster_RxHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MASTER_RX, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MASTER_RX , handle = 0x%04x \n\r", CustomContext.CustomMaster_RxHdle);
  }
   ...
   *  MASTER_TX1
   */
  COPY_MASTER_TX1_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomMaster_ServiceHdle,
                          UUID_TYPE_128, &uuid,
                          SizeMaster_Tx11,
                          CHAR_PROP_READ,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomMaster_Tx11Hdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MASTER_TX11, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MASTER_TX11 , handle = 0x%04x \n\r", CustomContext.CustomMaster_Tx11Hdle);
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MASTER_RX:
      ret = aci_gatt_update_char_value(CustomContext.CustomMaster_ServiceHdle,
                                       CustomContext.CustomMaster_RxHdle,
                                       0, /* charValOffset */
                                       SizeMaster_Rx, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MASTER_RX command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MASTER_RX command\n\r");
      }
    ...
    case CUSTOM_STM_MASTER_TX11:
      ret = aci_gatt_update_char_value(CustomContext.CustomMaster_ServiceHdle,
                                       CustomContext.CustomMaster_Tx11Hdle,
                                       0, /* charValOffset */
                                       SizeMaster_Tx11, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MASTER_TX11 command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MASTER_TX11 command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MASTER_RX:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMaster_ServiceHdle, CustomContext.CustomMaster_RxHdle, SizeMaster_Rx, pPayload);
    ...
    case CUSTOM_STM_MASTER_TX11:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomMaster_ServiceHdle, CustomContext.CustomMaster_Tx11Hdle, SizeMaster_Tx11, pPayload);
